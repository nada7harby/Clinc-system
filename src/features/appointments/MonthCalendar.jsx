  ...
  return <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between bg-white/80 backdrop-blur-xl p-4 rounded-3xl border border-white shadow-glass">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-xl">
             ...
             <button onClick={goToToday} className="px-3 h-8 text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-white hover:shadow-sm transition-all">{t("features.appointments.monthcalendar.today")}</button>
      ...
      <div className="bg-white rounded-[40px] border border-slate-100 shadow-glass overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b border-slate-50">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(d => <div key={d} className="py-4 text-center">
        ...
        <div className="grid grid-cols-7 grid-rows-5 lg:grid-rows-6">
          ...
          return <div key={idx} onClick={() => onAddAppointment(dateStr)} className={classNames("min-h-[140px] p-3 border-r border-b border-slate-50 transition-all cursor-pointer relative group", isOutside ? "bg-slate-50/30 opacity-40" : "bg-white hover:bg-slate-50/50", isWeekend(day) && !isOutside ? "bg-slate-50/20" : "")}>
                <div className="flex items-center justify-between mb-2">
                  <span className={classNames("text-xs font-black h-7 w-7 flex items-center justify-center rounded-full transition-all", isToday(day) ? "bg-brand-500 text-white shadow-glow" : "text-slate-400 group-hover:text-slate-900")}>
                    {format(day, "d")}
                  </span>
                  {dayAppts.length > 0 && <div className="h-1.5 w-1.5 rounded-full bg-brand-500 animate-pulse"></div>}
                </div>

                <div className="space-y-1.5">
                   {dayAppts.slice(0, 3).map(appt => <MiniAppointmentCard key={appt.id} appointment={appt} onClick={onEditAppointment} />)}
                   {dayAppts.length > 3 && <div onClick={e => {
                e.stopPropagation();
                setSelectedDay({
                  date: dateStr,
                  appts: dayAppts
                       ...
                       + {dayAppts.length - 3}{t("features.appointments.monthcalendar.more")}</div>}
                </div>
      ...
      <AnimatePresence>
        {selectedDay && <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
             <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          opacity: 0
        }} onClick={() => setSelectedDay(null)} className="absolute inset-0 bg-slate-950/40 backdrop-blur-sm" />
             <motion.div initial={{
          opacity: 0,
          scale: 0.95,
          y: 20
          ...
          y: 20
                ...
                <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                   <div>
                      ...
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                        {selectedDay.appts.length}{t("features.appointments.monthcalendar.appointmentsScheduled")}</p>
                   </div>
                ...
                <div className="p-6 max-h-[400px] overflow-y-auto space-y-3">
                   {selectedDay.appts.map(appt => <div key={appt.id} onClick={() => {
              ...
              setSelectedDay(null);
                ...
                <div className="p-4 bg-slate-50 flex justify-center">
                   <button onClick={() => {
              onAddAppointment(selectedDay.date);
              setSelectedDay(null);
            }} className="flex items-center gap-2 text-[10px] font-black text-brand-600 uppercase tracking-widest hover:underline">
                      <Icon name="faPlus" />{t("features.appointments.monthcalendar.addNewForThisDay")}</button>
                </div>
             </motion.div>
          </div>}
      </AnimatePresence>
