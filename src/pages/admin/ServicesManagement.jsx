...
function ServicesManagement() {
  ...
  const { data: servicesData, isLoading } = useServices({ search: searchTerm });
  ...
  const handleOpenModal = (service = null) => {
    ...
    if (service) {
      ...
      reset({ name: "", price: "", duration: "", category: "General", status: "active", description: "" });
    }
    setIsModalOpen(true);
  };
  ...
  const onSubmit = (data) => {
    if (editingService) {
      updateService({ id: editingService.id, data }, { onSuccess: handleCloseModal });
    } else {
      createService(data, { onSuccess: handleCloseModal });
    }
  };
    ...
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
           ...
           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">/ session</span>
        </div>
      )
    },
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-2 text-slate-500">
           ...
           <span className="text-sm font-bold">{row.duration} min</span>
        </div>
      )
    },
    { 
      header: "Status", 
      render: (row) => (
        <Badge tone={row.status === "active" ? "success" : "secondary"}>
          {row.status}
        </Badge>
      )
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => { if(confirm("Are you sure?")) deleteService(row.id); }} className="h-9 w-9 p-0 rounded-xl hover:bg-rose-50 hover:text-rose-600">
  ...
  return (
    <div className="space-y-10 pb-12">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-slate-950">Service Catalog</h1>
          <p className="mt-2 text-lg font-medium text-slate-500">Define treatment pricing, session durations, and clinical categories.</p>
        </div>
        <Button onClick={() => handleOpenModal()} className="h-12 px-8 rounded-2xl shadow-xl shadow-brand-500/20">
          <Icon name="faPlus" className="mr-3" />
          Create New Service
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
         ...
         <div className="space-y-8">
            <Card title="Catalog Stats" description="Summary of clinical offerings.">
               <div className="space-y-4 mt-4">
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                     <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Total Active</span>
            ...
            <Card className="bg-slate-950 text-white border-none shadow-xl shadow-slate-950/20">
               ...
               <p className="text-slate-400 text-xs leading-relaxed">
                  Regularly update your service prices to stay competitive. You can also link specific doctors to premium services.
               </p>
            </Card>
         </div>
      </div>
      ...
      <Modal 
        ...
        title={editingService ? "Update Treatment Details" : "Define New Treatment"}
        size="md"
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold tracking-tight text-slate-700 ml-1">Category</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold tracking-tight text-slate-700 ml-1">Status</label>
              <select 
                className="w-full h-12 rounded-2xl border-2 border-slate-100 bg-slate-50/50 px-4 text-sm font-bold outline-none focus:border-brand-500 focus:bg-white focus:ring-4 focus:ring-brand-500/10 transition-all"
          ...
          <div className="space-y-2">
             <label className="text-sm font-bold tracking-tight text-slate-700 ml-1">Description (Optional)</label>
             <textarea 
               className="w-full h-24 rounded-2xl border-2 border-slate-100 bg-slate-50/50 p-4 text-sm font-medium outline-none focus:border-brand-500 focus:bg-white focus:ring-4 focus:ring-brand-500/10 transition-all"
          ...
          <div className="mt-8 flex justify-end gap-3 border-t border-slate-50 pt-8">
            <Button variant="ghost" onClick={handleCloseModal} className="h-12 px-8">Cancel</Button>
