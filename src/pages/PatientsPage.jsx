...
function PatientsPage() {
  ...
  const { data: patientsData, isLoading } = usePatients({ search: searchTerm });
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-wrap gap-2">
          <Badge
            tone="primary"
            className="bg-brand-50 text-brand-700 border-brand-100 px-3"
          >
            Type: {row.bloodType}
          </Badge>
          {row.medicalHistory && (
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <Button
            ...
            className="h-10 px-4 rounded-xl text-xs gap-2"
          >
          ...
          {(user.role === ROLES.ADMIN || user.role === ROLES.RECEPTIONIST) && (
            <Button
              ...
              size="sm"
              onClick={() => {
                if (confirm("Permanently archive this patient record?"))
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-lg font-medium text-slate-500">
            Comprehensive management of digital health identities and histories.
          </p>
        </div>
        {(user.role === ROLES.ADMIN || user.role === ROLES.RECEPTIONIST) && (
          <Button className="h-12 px-8 rounded-2xl shadow-xl shadow-brand-500/20 gap-3">
      ...
      <Card className="p-0 overflow-hidden" variant="premium">
        <div className="bg-slate-50/50 p-6 border-b border-slate-100">
          <div className="relative max-w-2xl">
            ...
            <input
              type="text"
              placeholder="Search by name, biometric ID, phone, or email..."
        ...
        <div className="p-4">
          <Table
            ...
            isLoading={isLoading}
            emptyMessage="No clinical records matching your parameters."
          />
        </div>
      </Card>
              ...
              <div className="flex items-center gap-6 p-6 rounded-3xl bg-slate-50 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-500">
                    ID: PAT-{selectedPatient.id.toUpperCase()}
                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <section className="space-y-4">
                  <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                    <Icon name="faNotesMedical" className="text-brand-500" />
                    Medical History
                  </h3>
                  <div className="p-6 rounded-3xl bg-white border border-slate-100 shadow-sm min-h-[150px]">
                    <p className="text-sm font-medium text-slate-600 leading-relaxed">
                      {selectedPatient.medicalHistory ||
                        "No historical medical conditions recorded for this patient."}
                    </p>
                  </div>
                </section>
                ...
                <section className="space-y-4">
                  <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                    <Icon name="faClockRotateLeft" className="text-brand-500" />
                    Timeline
                  </h3>
                      ...
                      <div key={i} className="relative pl-10">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-500 uppercase mt-1">
                            Operator: {event.doctor} • {event.type}
                          </p>
                        </div>
                      </div>
                    ))}
