...
const QUEUE_STAGES = [
  { id: "waiting", label: "Waiting Room", icon: "faCouch", color: "amber" },
  { id: "triage", label: "Triage / Vitals", icon: "faHeartPulse", color: "blue" },
  { id: "doctor", label: "Doctor's Office", icon: "faUserDoctor", color: "brand" },
  { id: "checkout", label: "Checkout", icon: "faCashRegister", color: "emerald" },
];

// ── Insurance Provider Database (simulated) ───────────────────────────────────
const INSURANCE_PROVIDERS = [
  { id: "axa", name: "AXA Insurance", coverageRate: 0.80, copayFlat: 50 },
  { id: "bupa", name: "Bupa Health", coverageRate: 0.70, copayFlat: 75 },
  { id: "metlife", name: "MetLife", coverageRate: 0.85, copayFlat: 30 },
  { id: "globemed", name: "Globemed Alliance", coverageRate: 0.60, copayFlat: 100 },
];
...
function ReceptionistDashboard() {
  ...
  const [queuePatients, setQueuePatients] = useState([
    { id: "q1", name: "Ava Moore", service: "Cardiac Follow-up", doctor: "Dr. Ahmed", time: "09:30", stage: "waiting" },
    { id: "q2", name: "Marcus Hill", service: "Diagnostics Review", doctor: "Dr. Sarah", time: "10:00", stage: "waiting" },
    { id: "q3", name: "Lina Ortiz", service: "Prescription Refill", doctor: "Dr. Ahmed", time: "10:30", stage: "triage" },
    { id: "q4", name: "Omar Khalil", service: "Dermatology Check", doctor: "Dr. Sarah", time: "11:00", stage: "doctor" },
    { id: "q5", name: "Fatma Hassan", service: "Pediatrics Visit", doctor: "Dr. Ahmed", time: "11:30", stage: "checkout" },
  ]);
  ...
  const { register: patientRegister, handleSubmit: patientSubmit, reset: patientReset, setValue: setPatientValue } = useForm();
  ...
  const handleDrop = useCallback((stageId) => {
    if (draggedPatient && draggedPatient.stage !== stageId) {
      setQueuePatients(prev =>
        prev.map(p => p.id === draggedPatient.id ? { ...p, stage: stageId } : p)
      );
      const stageLabel = QUEUE_STAGES.find(s => s.id === stageId)?.label;
      toast.success(`${draggedPatient.name} moved to ${stageLabel}`);
    }
  ...
  const handleVerifyInsurance = () => {
    if (!selectedInsuranceProvider || !insurancePolicyId.trim()) {
      toast.error("Please select a provider and enter a policy ID.");
      return;
    }
    setIsVerifying(true);
    setCopayResult(null);
    ...
    setTimeout(() => {
      ...
      toast.success("Insurance verified & copay calculated!");
    }, 1500);
  };
  ...
  const handleStartOCR = () => {
    ...
    const addLog = (msg, delay) => new Promise(resolve => {
      setTimeout(() => { setOcrLogs(prev => [...prev, msg]); resolve(); }, delay);
    });

    addLog("[0.5s] 📷 Camera feed detected. Aligning document edges...", 500)
      .then(() => addLog("[1.2s] 🔲 ID card detected: National ID Card (Egypt)", 700))
      .then(() => { setOcrState("parsing"); return addLog("[1.8s] 🤖 Running Tesseract OCR engine on captured frame...", 600); })
      .then(() => addLog("[2.4s] 📖 Extracting: Full Name → \"أحمد محمد يوسف\" → Ahmed Mohamed Youssef", 600))
      .then(() => addLog("[3.0s] 📖 Extracting: National ID → 29901150100834", 600))
      .then(() => addLog("[3.5s] 📖 Extracting: Date of Birth → 1999-01-15", 500))
      .then(() => addLog("[4.0s] 📖 Extracting: Gender → Male", 500))
      .then(() => addLog("[4.5s] ✅ OCR extraction complete. All fields parsed with 98.7% confidence.", 500))
      .then(() => {
        setOcrState("done");
        const result = {
        ...
        toast.success("ID fields auto-populated! Review and submit.");
      });
  };

  // Tab colors helper
  const tabStyle = (tabId) => classNames(
    "h-11 px-5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2",
    activeTab === tabId
      ? "bg-brand-500 text-white shadow-lg shadow-brand-500/25"
      : "bg-white border border-slate-100 text-slate-500 hover:border-brand-200 hover:text-slate-700"
  );
    ...
    <div className="space-y-10 pb-12">
      <header className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-500 font-medium">Manage patient flow, billing, and registration with precision.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" size="lg" className="rounded-2xl gap-2 shadow-sm" onClick={() => setIsInsuranceModalOpen(true)}>
            <Icon name="faFileInvoiceDollar" />
            Insurance POS
          </Button>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button onClick={() => setActiveTab("schedule")} className={tabStyle("schedule")}>
          <Icon name="faTableList" /> Live Schedule
        </button>
        <button onClick={() => setActiveTab("queue")} className={tabStyle("queue")}>
          <Icon name="faGripVertical" /> Patient Flow Board
        </button>
      </div>
                  ...
                  render: (row) => (
                    <div className="flex items-center gap-2">
                      {row.status === APPOINTMENT_STATUS.PENDING && (
                        <Button variant="success" size="sm" className="h-8 px-3 text-[10px] font-black uppercase tracking-widest" onClick={() => handleCheckIn(row.id)}>
                          Check-in
                        </Button>
                      )}
                      {row.paymentStatus !== "paid" && row.status !== APPOINTMENT_STATUS.CANCELLED && (
                        <Button
                          ...
                          onClick={() => { setSelectedAppointmentForPayment(row); setIsPaymentModalOpen(true); }}
                        >
                          <Icon name="faMoneyBillWave" />
                          Collect
                        </Button>
                      )}
                      <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-lg text-slate-400 hover:text-rose-500" onClick={() => { if(confirm("Cancel appointment?")) cancelAppt(row.id); }}>
                        <Icon name="faXmark" />
                      </Button>
                    </div>
                  ),
                ...
                {stats.total > 15 ? (
                  <div className="p-4 rounded-2xl bg-rose-50 border border-rose-100">
                    <p className="text-xs font-black text-rose-700 uppercase tracking-widest mb-1 flex items-center gap-2">
                      <Icon name="faCircleExclamation" /> High Load Detected
                    </p>
                  ...
                  <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
                    <p className="text-xs font-black text-emerald-700 uppercase tracking-widest mb-1 flex items-center gap-2">
                      <Icon name="faCheckCircle" /> Schedule Optimal
                    </p>
      ...
      {activeTab === "queue" && (
        <div className="space-y-4">
          <p className="text-xs font-bold text-slate-500">
            <Icon name="faInfoCircle" className="mr-1" />
            Drag patient cards between columns to move them through the clinic flow.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {QUEUE_STAGES.map((stage) => {
              ...
              const colorMap = {
                amber: { bg: "bg-amber-50/50", border: "border-amber-200", text: "text-amber-600", iconBg: "bg-amber-100" },
                blue: { bg: "bg-blue-50/50", border: "border-blue-200", text: "text-blue-600", iconBg: "bg-blue-100" },
                brand: { bg: "bg-brand-50/50", border: "border-brand-200", text: "text-brand-600", iconBg: "bg-brand-100" },
                emerald: { bg: "bg-emerald-50/50", border: "border-emerald-200", text: "text-emerald-600", iconBg: "bg-emerald-100" },
              };
              ...
              return (
                <div
                  ...
                  className={classNames(
                    "rounded-3xl border-2 p-4 min-h-[280px] transition-all duration-300",
                    isOver ? `${c.border} ${c.bg} scale-[1.02] shadow-xl` : "border-slate-100 bg-white/50"
                  )}
                >
                  {/* Column Header */}
                  <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="space-y-3">
                    <AnimatePresence>
                      {stagePatients.map((patient) => (
                        <MotionDiv
                          ...
                          className={classNames(
                            "rounded-2xl border bg-white p-3.5 cursor-grab active:cursor-grabbing transition-all hover:shadow-lg hover:-translate-y-0.5 group",
                            draggedPatient?.id === patient.id ? "opacity-50 border-brand-300 shadow-lg" : "border-slate-100 shadow-sm"
                          )}
                        >
                          <div className="flex items-center gap-3">
                    ...
                    {stagePatients.length === 0 && (
                      <div className={classNames(
                        ...
                        isOver ? `${c.border}` : "border-slate-200"
                      )}>
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      <Modal isOpen={isNewPatientModalOpen} onClose={() => { setIsNewPatientModalOpen(false); setOcrState("idle"); setOcrLogs([]); setOcrResult(null); }} title="Register New Patient" size="md">
        <form onSubmit={patientSubmit(handlePatientCreate)} className="space-y-6 pt-4">
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-slate-600">Smart ID Scanner (OCR)</span>
              </div>
              {ocrState === "idle" && (
                <Button type="button" onClick={handleStartOCR} variant="outline" className="h-9 px-3 rounded-xl text-[10px] font-black uppercase tracking-widest gap-1.5">
                  <Icon name="faCamera" />
                  Scan ID Card
                </Button>
              )}
              {ocrState === "done" && (
                <Badge tone="success" className="uppercase font-black text-[9px] tracking-widest">
                  <Icon name="faCheckCircle" className="mr-1" /> Extracted
                </Badge>
              )}
            </div>
            ...
            {/* OCR Terminal Logs */}
            {ocrLogs.length > 0 && (
              <div className="bg-slate-900 text-emerald-400 font-mono text-[10px] p-3 rounded-xl max-h-[120px] overflow-y-auto border border-white/5 shadow-inner leading-relaxed">
                {ocrLogs.map((log, i) => (
                  <div key={i}>&gt; {log}</div>
                ))}
              </div>
            ...
            {ocrState !== "idle" && ocrState !== "done" && (
              <div className="flex items-center gap-2 text-brand-600 font-black uppercase text-[10px] tracking-widest">
          ...
          <div className="space-y-4">
            ...
            <Input label="National ID (Optional)" {...patientRegister("nationalId")} placeholder="e.g. 29901150100834" />
            <div className="space-y-2">
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
            <Button variant="ghost" type="button" onClick={() => { setIsNewPatientModalOpen(false); setOcrState("idle"); setOcrLogs([]); setOcrResult(null); }}>Discard</Button>
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      <Modal isOpen={isInsuranceModalOpen} onClose={resetInsuranceModal} title="Insurance POS & Copay Calculator" size="md">
        <div className="space-y-6 pt-4 text-slate-700">
          <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 text-center">
            ...
            <h5 className="text-sm font-bold text-slate-800">Instant Insurance Verification</h5>
            <p className="text-xs text-slate-500 font-medium mt-1">Verify patient coverage and calculate copayment share in seconds.</p>
          </div>

          {/* Provider & Policy Inputs */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Insurance Provider</label>
              <select
                value={selectedInsuranceProvider}
                onChange={(e) => { setSelectedInsuranceProvider(e.target.value); setInsuranceVerified(false); setCopayResult(null); }}
              ...
              >
                ...
                {INSURANCE_PROVIDERS.map(p => (
                  <option key={p.id} value={p.id}>{p.name} ({(p.coverageRate * 100)}% Coverage)</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Policy Number</label>
              <input
                type="text"
                value={insurancePolicyId}
                onChange={(e) => { setInsurancePolicyId(e.target.value); setInsuranceVerified(false); setCopayResult(null); }}
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Service Amount ($)</label>
            <input
              type="number"
              value={insuranceServiceAmount}
              onChange={(e) => { setInsuranceServiceAmount(e.target.value); setInsuranceVerified(false); setCopayResult(null); }}
          ...
          {!insuranceVerified && (
            ...
            >
              {isVerifying ? (
                <><Icon name="faSpinner" className="animate-spin" /> Verifying Coverage...</>
              ) : (
                <><Icon name="faShieldHalved" /> Verify Insurance & Calculate Copay</>
              )}
            </Button>
          )}
          ...
          {copayResult && (
            ...
            >
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50/30 p-3 flex items-center gap-2 text-emerald-600 text-xs font-bold">
                ...
                <span>Policy verified! Approval Ref: {copayResult.approvalRef}</span>
              </div>

              <div className="rounded-3xl border border-slate-200 bg-white p-5 space-y-3 shadow-sm">
                <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                  <span className="text-xs font-black uppercase tracking-widest text-slate-400">Coverage Breakdown</span>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-emerald-600">
                    <span className="font-medium">Insurance Covers</span>
                    <span className="font-bold">${copayResult.insurancePays}</span>
                  </div>
                  <div className="border-t border-slate-100 pt-2 flex justify-between items-center">
                    <span className="font-black uppercase text-[10px] tracking-widest text-slate-600">Patient Copay Due</span>
              ...
              <Button onClick={resetInsuranceModal} className="w-full h-12 rounded-2xl gap-2 shadow-md bg-brand-500 text-white">
                <Icon name="faCheckCircle" /> Apply to Patient Account
              </Button>
            </MotionDiv>
          )}
        </div>
      ...
      {selectedAppointmentForPayment && (
        <PaymentModal
          ...
          onClose={() => { setIsPaymentModalOpen(false); setSelectedAppointmentForPayment(null); }}
