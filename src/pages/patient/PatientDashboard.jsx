...
function getGreeting(t) {
  ...
  if (h < 12) return {
    text: t("patient.greetings.morning"),
    emoji: "☀️"
  };
  if (h < 17) return {
    text: t("patient.greetings.afternoon"),
    emoji: "🌤️"
  };
  return {
    text: t("patient.greetings.evening"),
    emoji: "🌙"
  };
}
...
function getDaysUntil(t, dateStr) {
  ...
  if (diff > 0 && diff < 31) return t("patient.days.inDays", {
    count: diff
  });
  return null;
}
...
function Skeleton({
  ...
  return <div className={`relative overflow-hidden bg-slate-100 ${className}`} style={{
...
function PatientDashboard() {
  ...
  const firstName = user?.name?.split(" ")[0] || t("common.there", {
    defaultValue: t("pages.patient.patientdashboard.there")
  });
  ...
  const stats = useMemo(() => {
    ...
    const upcoming = all.filter(a => a.status === APPOINTMENT_STATUS.PENDING || a.status === APPOINTMENT_STATUS.CONFIRMED).sort((a, b) => new Date(`${a.date}T${a.time}`) - new Date(`${b.date}T${b.time}`));
    return {
  ...
  return <div className="relative  space-y-5 pb-28">
      {/* ── 1. PERSONALIZED HEADER ────────────────────────────────────────── */}
      <motion.header {...fadeUp(0)} className="relative overflow-hidden rounded-[28px] p-6" style={{
      background: "linear-gradient(145deg, #fffdf2 0%, #eef5ff 48%, #fff3b4 100%)"
    }}>
        {/* Decorative blobs */}
        <Link className="pointer-events-none absolute -right-16 -top-16 h-52 w-52 rounded-full blur-3xl" style={{
        background: "rgba(31,64,114,0.14)"
      }} />
        <div className="pointer-events-none absolute -bottom-12 -left-10 h-44 w-44 rounded-full blur-3xl" style={{
        background: "rgba(140,200,170,0.14)"
      }} />
        ...
        <div className="relative flex items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div className="relative shrink-0">
              <div className="flex h-[58px] w-[58px] items-center justify-center rounded-full text-xl font-black text-white shadow-md ring-[3px] ring-white" style={{
        ...
        <div className="relative mt-4 flex items-center gap-2.5 rounded-xl border border-white/70 bg-white/50 px-3.5 py-2.5 backdrop-blur-sm">
          ...
          background: "rgba(31,64,114,0.1)"
        }}>
            <Icon name="faShieldHalved" className="text-[11px] text-brand-600" />
          </div>
      ...
      <motion.section {...fadeUp(0.05)}>
        <div className="grid grid-cols-4 gap-3">
          {QUICK_ACTIONS.map(action => <Link key={action.labelKey} to={action.to} className="group">
              <div className={`flex flex-col items-center gap-2 rounded-2xl p-3 text-center transition-all active:scale-95 ${action.isPrimary ? "text-white shadow-md" : "border border-slate-100 bg-white hover:border-brand-200 hover:shadow-sm"}`} style={action.isPrimary ? {
            background: "linear-gradient(135deg, #1f4072, #4f86cf)",
            boxShadow: "0 4px 16px rgba(31,64,114,0.3)"
          } : {}}>
                <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${action.isPrimary ? "bg-white/20" : "bg-brand-50"}`}>
                  <Icon name={action.icon} className={`text-sm ${action.isPrimary ? "text-white" : "text-brand-500"}`} />
                </div>
                <span className={`text-[10px] font-black leading-tight ${action.isPrimary ? "text-white/90" : "text-slate-500"}`}>
                  {t(action.labelKey)}
                </span>
              </div>
            </Link>)}
      ...
      <motion.section {...scaleIn(0.1)}>
        <div className="mb-3 flex items-center justify-between px-0.5">
          ...
          <Link to={ROUTES.myAppointments} className="text-[10px] font-black uppercase tracking-widest text-brand-600 hover:underline">
            {t("patient.dashboard.seeAll")} →
          </Link>
        </div>

        <AnimatePresence mode="wait">
          {isLoading ? <motion.div key="hero-sk" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          opacity: 0
            ...
            </motion.div> : stats.next ? (/* ── Has appointment ── */
        <motion.div key="hero-appt" initial={{
          opacity: 0,
          y: 12
        }} animate={{
          opacity: 1,
          ...
          duration: 0.4,
          ease: [0.22, 1, 0.36, 1]
        }} className="relative overflow-hidden rounded-[28px] p-6 text-white" style={{
          background: "linear-gradient(145deg, #1f4072 0%, #122746 65%, #081426 100%)"
        }}>
              {/* Glow orbs */}
              <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full blur-3xl" style={{
            background: "rgba(255,243,180,0.2)"
          }} />
              <div className="pointer-events-none absolute -bottom-12 left-8 h-48 w-48 rounded-full blur-3xl" style={{
            background: "rgba(80,140,110,0.12)"
          }} />
              ...
              <div className="relative mb-5 flex items-center justify-between">
                ...
                {daysUntil && <div className="rounded-full px-3 py-1 text-[10px] font-black text-white/70" style={{
              ...
              {/* Doctor info card */}
              <div className="relative mt-5 rounded-2xl p-3.5" style={{
            background: "rgba(255,255,255,0.07)",
            border: "1px solid rgba(255,255,255,0.09)"
          }}>
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-black text-white" style={{
                background: "linear-gradient(135deg,#1f4072,#4f86cf)"
              }}>
                    {stats.next.doctorName?.charAt(0) || "D"}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[13px] font-black text-white">{t("pages.patient.patientdashboard.dr")}{stats.next.doctorName}
                    </p>
                    <p className="text-[11px] text-white/40">
                      {stats.next.specialty || t("patient.header.generalPractitioner", {
                    defaultValue: t("pages.patient.patientdashboard.generalPractitioner")
                  })}
                    </p>
                  </div>
                  <span className="shrink-0 rounded-xl px-2.5 py-1 text-[10px] font-black text-brand-300" style={{
                background: "rgba(255,243,180,0.16)",
                border: "1px solid rgba(255,243,180,0.24)"
              }}>
                    {stats.next.serviceName || t("patient.header.consultation", {
                  defaultValue: t("pages.patient.patientdashboard.consultation")
                })}
                  </span>
                </div>
              </div>
              ...
              <div className="relative mt-4 flex gap-2.5">
                <Link to={ROUTES.myAppointments} className="flex-1">
                  <button className="w-full rounded-2xl py-3 text-[11px] font-black uppercase tracking-wider text-white transition-all hover:opacity-90 active:scale-95" style={{
                background: "linear-gradient(135deg, #1f4072, #4f86cf)"
              }}>
                    {t("patient.header.manageBooking")}
                  </button>
                </Link>
                <button className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl text-white transition-all hover:bg-white/15 active:scale-95" style={{
        ...
        <motion.div key="hero-empty" initial={{
          opacity: 0,
          scale: 0.96
        }} animate={{
          opacity: 1,
          ...
          opacity: 0
              ...
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full" style={{
            background: "linear-gradient(135deg, #fffdf2, #fff3b4)"
          }}>
                <Icon name="faCalendarPlus" className="text-2xl text-brand-400" />
              </div>
              ...
              <Link to={ROUTES.bookAppointment} className="mt-6">
                <button className="rounded-2xl px-7 py-3.5 text-[13px] font-black text-white shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl active:scale-95" style={{
              background: "linear-gradient(135deg, #1f4072, #4f86cf)",
      ...
      <motion.section {...fadeUp(0.15)}>
        ...
        <div className="grid grid-cols-2 gap-3">
          {VITALS.map(v => <div key={v.key} className={`relative overflow-hidden rounded-2xl p-4 ring-1 ${v.ring}`} style={{
          background: `linear-gradient(145deg, ${v.gradFrom}, ${v.gradTo})`
        }}>
              {/* Decoration circle */}
              <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full blur-2xl" style={{
            background: "rgba(255,255,255,0.7)"
          }} />

              <div className="relative flex items-start justify-between">
                <div className={`flex h-9 w-9 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ${v.ring}`}>
          ...
          <Link to={ROUTES.medicalHistory} className="group">
            <div className="flex flex-col justify-between rounded-2xl border border-brand-100 bg-gradient-to-br from-brand-50 to-rose-50/30 p-4 transition-all hover:-translate-y-0.5 hover:shadow-md">
              <div className="mb-4 flex h-9 w-9 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ring-brand-100">
                <Icon name="faClipboardList" className="text-sm text-brand-500" />
              </div>
              <div>
                ...
                <div className="mt-1 text-[28px] font-black leading-none text-slate-900">
                  {isLoading ? <span className="inline-block h-7 w-10 animate-pulse rounded-lg bg-brand-100" /> : stats.totalVisits}
                </div>
      ...
      <motion.section {...fadeUp(0.22)}>
        <div className="mb-3 flex items-center justify-between px-0.5">
          <div className="flex items-center gap-2">
            ...
            {!isLoading && stats.upcoming.length > 0 && <span className="rounded-full bg-brand-100 px-2 py-0.5 text-[9px] font-black text-brand-600">
                {stats.upcoming.length}
              </span>}
          </div>
          {stats.upcoming.length > 0 && <Link to={ROUTES.myAppointments} className="text-[10px] font-black uppercase tracking-widest text-brand-600 hover:underline">
              {t("patient.dashboard.all")} →
            </Link>}
        </div>

        <div className="space-y-2.5">
          {isLoading ? [1, 2].map(i => <div key={i} className="h-[76px] rounded-2xl">
                <Skeleton className="h-full rounded-2xl" />
              </div>) : stats.upcoming.length > 0 ? stats.upcoming.map((appt, i) => {
          const d = formatApptDate(appt.date, locale);
          const s = STATUS_MAP[appt.status] || STATUS_MAP.pending;
          return <motion.div key={appt.id} initial={{
            opacity: 0,
            x: -10
          }} animate={{
            opacity: 1,
            ...
            ease: [0.22, 1, 0.36, 1]
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 flex items-center gap-1 text-[11px] text-slate-400">
                      <Icon name="faUserMd" className="text-[9px]" />{t("pages.patient.patientdashboard.dr2")}{appt.doctorName}
                  ...
                  <div className="flex flex-col items-end gap-1.5">
                    <span className={`rounded-lg border px-2 py-0.5 text-[9px] font-black uppercase tracking-widest ${s.badge}`}>
                      {t(s.labelKey)}
                    </span>
                    <Icon name="faChevronRight" className="text-[9px] text-slate-300 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </motion.div>;
      ...
      <motion.section {...fadeUp(0.26)}>
        ...
        background: "linear-gradient(145deg, #1f4072 0%, #122746 100%)"
          ...
          {/* Glow */}
          <div className="pointer-events-none absolute -right-10 -top-10 h-36 w-36 rounded-full blur-3xl" style={{
          background: "rgba(255,243,180,0.22)"
        }} />
          ...
          <div className="relative flex items-center gap-3.5">
            ...
            <button className="ml-auto shrink-0 rounded-xl px-4 py-2.5 text-[11px] font-black uppercase tracking-wider text-white transition-all hover:-translate-y-0.5 active:scale-95" style={{
          ...
          {/* Chat entry point */}
          <div className="relative mt-4 border-t pt-4" style={{
          borderColor: "rgba(255,255,255,0.06)"
            ...
            <button className="flex w-full items-center justify-between">
              ...
              <Icon name="faChevronRight" className="text-[9px] text-white/20" />
            </button>
          </div>
        </div>
      </motion.section>
      ...
      <Link to={ROUTES.bookAppointment} className="fixed bottom-8 right-6 z-50 lg:right-10">
        ...
        scale: 0.95
      }} className="flex h-14 w-14 items-center justify-center rounded-full text-white" style={{
        background: "linear-gradient(135deg, #1f4072, #4f86cf)",
